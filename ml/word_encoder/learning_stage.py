...
from ml.word_encoder.dataset_class.dataclass_word_decoder import CRNNWordDataset
from ml.word_encoder.models import CRNNWordEncoder, model_word_encoder_code
from ml.word_encoder.metrics import calculate_cer, calculate_wer, decode_predictions
from ml.word_encoder.utils import plot_lr_chronology, plot_loss_dynamics, plot_metrics_dynamics
...
# ======================================================================================================================
    ...
    model = CRNNWordEncoder(num_classes, hidden_size, lstm_layers, dropout, pretrained_backbone).to(env.device)


    loss_func = CTCLoss(blank=0, zero_infinity=True)
    ...
    threshold_loss = 0.01
    early_stopping = 12
    models_dir = WORKDIR / 'ml' / 'word_encoder' / 'model_weights' / f'{datetime.now().strftime("%Y%m%d-%H%M%S")}'
...
# ======================================================================================================================
    ...
    train_loss_list, val_loss_list, lr_list, cer_list, wer_list, min_loss = [], [], [], [], [], None
    ...
    for epoch in range(1, epochs + 1):
        ...
        log_event(f"\033[32mTRAINING\033[0m | Epoch {epoch} | train_loss=\033[33m{last_train_loss:.4f}\033[0m")
    ...
    # ==================================================================================================================
        ...
        log_event(f"\033[34mVALIDATION\033[0m Epoch {epoch} | val_loss=\033[31m{last_val_loss:.4f}\033[0m | CER=\033[33m{cer:.2f}%\033[0m | WER=\033[36m{wer:.2f}%\033[0m | LR={lr:.6f}")
        ...
        min_loss = last_val_loss if min_loss is None else min_loss
        if min_loss - min_loss * threshold_loss > last_val_loss:
            min_loss = last_val_loss
            ...
            plateau_loss_epochs = 0
            log_event(f'На эпохе - \033[35m{epoch}\033[0m модель сохранена | val_loss=\033[34m{last_val_loss:.4f}\033[0m, CER=\033[36m{cer:.2f}\033[0m%, WER=\033[34m{wer:.2f}\033[0m%', level='WARNING')
