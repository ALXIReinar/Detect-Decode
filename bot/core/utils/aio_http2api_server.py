...
class ApiServerConn:
    ...
    async def imgs2inference(self, tg_id: int, media_list: list[str]):
        """
        ...
        async with self.aio_http_session.post(
                '/api/v1/images/to_ocr_inference',
                json={'tg_id': tg_id, 'file_ids': media_list}
        ) as resp:
    ...
    async def rate_img_inference(self, img_id: int | str, rate: int | str):
        ...
        async with self.aio_http_session.put(
            '/api/v1/images/inference/ocr/rate',
